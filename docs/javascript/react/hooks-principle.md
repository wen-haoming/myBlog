...
> 以 const [num,setNum] = useState(0) 为例，每次组件渲染的时候， `useState` 会把上一次的结果都在内部计算出来，而 `setNum` 只是在上一次更新的时候都记录下来并且放到当前 hook 的链表中。
...
另外我们需要全局声明一个变量 `workInProgressHooks` 的指针来指向当前需要运行的 hook
